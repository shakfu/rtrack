...
- **GUI: drag trim, slice and loop edges on the sample waveform.** In the Samples tab, the edges of the viewed slice set can be dragged, and so can the viewed slot's loop points. An edge shared by two slices moves both, so the set stays contiguous; `SampleBank::move_boundary` holds that rule and the clamping that keeps every span at least one frame long. Tabs mark what can be grabbed: trim and slice edges at the top, loop points at the bottom. A loop start at its default sits on the trim start, and the half of the waveform pressed picks which moves. Playback follows the drag, and the whole gesture is one undo step. Alt-drag moves one slice only (`Coupling::Alone`): shortening a slice's tail then leaves a gap, rather than moving the next slice's start back into the tail. The TUI's trim and loop fields now use the same rule. Arrow keys move a shared edge on both slices, Shift with an arrow moves one, and a loop point can no longer be stored outside the span it plays in.

- **Snap loop points to zero crossings.** `z` in the TUI sample editor, and *Snap to zero* in the GUI instrument editor, move both loop points to the nearest rising zero crossing within 20 ms of each (`Sample::snap_loop_to_zero_crossings`). Both frames at the seam are then near zero on the same slope. A 440 Hz sine looped over 49.9 periods had a step at the seam 10.4 times the 99th-percentile step; after snapping, 1.4 times. The snap is one undo step. It moves value and slope into line, not the waveform shape, so a seam between unlike material can still be heard; a crossfade would remove that.

- **`9xx` sample offset.** A note with `9xx` starts its sample `xx`/256 of the way into the played span: `9 80` starts halfway through a slice. The offset is relative to the span, not ProTracker's absolute `xx * 256` frames. Slices vary in length, and at 44.1 kHz the absolute form reaches only 1.5 s. It applies to samples only, and only with a note on the same row. `TrackerEvent::NoteOn` gains a `sample_offset` field, and `AudioCommand::SampleNoteOn` an `offset`; code that builds either needs the new field. `examples/sample-offset.rtrk`, generated by `cargo xtask regen-examples`, demonstrates it.
...
- **Looping samples interpolate across the loop seam.** Near `loop_end` the interpolator read the frames after it, and just after wrapping it read the frame before `loop_start`; neither plays next. At any rate other than 1.0 this bent the waveform once per pass. Measured on a seamless 8-period loop surrounded by constant audio, the output deviated from the ideal sine by up to 0.94; it now stays under 0.001. The frame before `loop_start` is still read before the first wrap, when it is the audio that just played.

- **A SoundFont that fails to load no longer silences everything.** `AudioEngine::new` fails when the `sf2` file cannot be opened, and both frontends treated that failure as having no audio at all. A config carried over from another machine played nothing, and the reason appeared only briefly in the TUI status bar and not at all in headless `--play`. Both frontends now call `AudioEngine::new_with_sf2_fallback`, which retries without the SoundFont and reports why it was dropped. The TUI shows that report after the song loads, and `--play` prints it to stderr. `AudioEngine::new` still fails on a bad path, for callers that need the SoundFont.

- **Re-slicing a sample into fewer pieces clears the old slices past the new ones.** `slice_sample` wrote only the slots the new division used, so going from 16 slices to 8 left slots 8-15 holding the 16-way slices, overlapping the new ones and still playable. The GUI re-slices on every count change, so dragging the count down produced this. With `SliceRange::Source`, same-file slices directly after the new ones are now cleared. A slot holding the file loaded whole, or an instrument renamed by hand, is kept. `Span` clears nothing: it replaces only the slots it writes. `Source` also started from the wrong slot. The TUI used the slot being edited, which left the earlier slices of the set in place. The GUI used the lowest slot holding the same file, which wrote over that file loaded whole elsewhere. Both now start from the first slot of the contiguous slice set (`SampleBank::slice_set_start`).

- **GUI: viewing a sample in the Samples tab no longer slices it.** The slicing controls apply when a setting changes, and the settings in force on arriving at a sample counted as a change. Selecting an unsliced sample cut it into eight. Those settings are now the baseline for that slice set; changing the count, sensitivity, mode or range still applies at once. A *Slice* button applies them by hand, shown only when the set does not already hold the slices they would produce. The comparison and the preview markers use the new `rtrack_core::sample::plan_slices`, which `slice_sample` also calls. The equal-slice markers previously rounded differently from slicing, off by up to `count - 2` frames.

- **Loop markers on a slice are drawn where playback loops.** The GUI instrument editor and the TUI sample waveform drew the stored loop points. Playback clamps them into the slice, so a looped slice with the default points showed its loop at the start of the whole file.
...
- **The slice preview is computed once per change, not once per redraw.** Both editors ran transient detection on every repaint: 3.8 ms on a 60 s sample, against a 5 ms TUI redraw during playback. `rtrack_core::sample::SlicePlanCache` keeps the planned spans until the sample, its trim or a slice setting changes; a cached call takes 3.3 ns. It holds the sample's buffer, not its address, so a reloaded file at a reused address cannot match. The TUI markers now come from the same plan as slicing, which drops the transient marker at the span start and fixes the equal-slice rounding described under Fixed.
