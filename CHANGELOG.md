...
- **The audio callback no longer allocates when the backend hands it an oversized buffer.** The scratch buffers are sized for `MAX_CALLBACK_FRAMES` (4096) at stream setup, but the callback called `ensure_capacity(frames)`, which grew them to fit anything larger. cpal asks a backend for a buffer size; it is not obliged to honour it, so this was reachable, and a blocking allocator call on the audio thread is exactly the dropout the buffering exists to prevent. The callback now renders a larger buffer in blocks the scratch space can already hold, which is free: the DSP is sample-wise and each buffer was already being rendered in segments so that commands land on their own frame (`audio/mod.rs`)
