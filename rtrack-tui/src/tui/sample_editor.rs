...
fn render_waveform(sample: &rtrack_core::sample::Sample, width: usize, height: usize) -> Vec<String> {
    ...
    for peak in peaks.iter().take(width) {
        let level = (*peak * height as f32) as usize;
        for (row_idx, row_str) in rows.iter_mut().enumerate() {
            let from_bottom = height - 1 - row_idx;
            if from_bottom < level {
                let intensity = ((*peak * (blocks.len() - 1) as f32) as usize)
                    .min(blocks.len() - 1);
                row_str.push(blocks[intensity]);
            } else {
                row_str.push(' ');
            }
        }
