...
use rtrack_core::audio::synth::{FilterType, Patch};
...
pub fn draw_synth_editor(f: &mut Frame, app: &App) {
    ...
    let inst_name = if app.core.instruments[slot].name.is_empty() {
        "---"
    } else {
        &app.core.instruments[slot].name
    };
    ...
    if let Some(ref params) = app.core.instruments[slot].synth_params {
