...
fn draw_status_bar(f: &mut Frame, app: &App, area: Rect, theme: &Theme) {
    ...
    let midi_status = if app.core.midi.send_error_count > 0 {
        Span::styled(
            format!(" MIDI:ERR({}) ", app.core.midi.send_error_count),
            Style::default().fg(theme.mode_insert),
        )
    } else if app.core.midi_connected() {
        Span::styled(
            format!(" MIDI:{} ", app.core.midi_port_display_name()),
...
fn draw_instrument_list(f: &mut Frame, app: &App, theme: &Theme) {
    ...
    let start = app.dialogs.instrument_cursor.saturating_sub(visible / 2);

    let items: Vec<ListItem> = (start..256.min(start + visible))
        .map(|i| {
            let inst = &app.core.instruments[i];
...
fn draw_track_config(f: &mut Frame, app: &App, theme: &Theme) {
    ...
    if is_synth {
        let inst_val = app.core.channels.get(ch).and_then(|c| c.default_instrument);
        let inst_display = match inst_val {
            Some(i) => {
                let inst = &app.core.instruments[i as usize];
...
fn draw_port_selector(f: &mut Frame, app: &App, theme: &Theme) {
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(i, name)| {
            ...
            } else if app.core.midi.port_name.as_deref() == Some(name) {
                Style::default().fg(theme.midi_connected)
            } else {
                Style::default().fg(theme.popup_text)
            };
...
fn draw_pattern_matrix(f: &mut Frame, app: &App, area: Rect, theme: &Theme) {
    ...
    let num_channels = app.core.song.channels;
    let ch_col_w: u16 = 5;
    ...
    let scroll_offset = if order_len <= data_height || app.matrix_cursor <= half {
        0
